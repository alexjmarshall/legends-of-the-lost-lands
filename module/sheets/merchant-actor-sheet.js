...
export class MerchantActorSheet extends ActorSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: 'systems/brigandine/templates/merchant-actor-sheet.html',
      width: 600,
      height: 600,
  ...
  async getData() {
    ...
    // item prices
    const merchantActor = context.actor.isToken
      ? context.actor
      : canvas.tokens.objects.children.find((t) => t.actor._id === context.actor._id && t.data.actorLink === true)
          ?.actor;
    const character = game.user.character;
    const attitude =
      merchantActor && character
        ? await reactionRoll(merchantActor, character, { showModDialog: false, showChatMsg: false })
