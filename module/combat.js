...
export async function attack(attacker, target, options) {
  ...
  const removedLocs = targetActor?.getFlag('brigandine', 'removedLocs') || [];
  ...
  if (!weapons.length) {
    ...
    if (doTargetUpdates && totalEnergyDrainDmg && targetActor) {
      const storedDamage = +targetActor.getFlag('brigandine', 'energyDrainDamage') || 0;
      await targetToken.actor.setFlag('brigandine', 'energyDrainDamage', storedDamage + totalEnergyDrainDmg);
    }
  ...
  const atkModes = isBow
    ? quiver.data.data.attributes.atk_modes?.value
        .split(',')
  ...
  const atkTime = weaponItem.data.data.atk_timing;
  let atkTimeText = '';
  ...
  if (Util.stringMatch(atkTime, 'riposte') || Util.stringMatch(atkTime, 'counter')) {
    ...
    // weaponItem._id && attackerItemUpdates.push({'_id': weaponItem._id, 'data.atk_timing': 'immediate'});
  }
  ...
  const wearingShield = actorItems.some(
  ...
  let dmgType = Constant.DMG_TYPES.includes(weapAttrs.dmg_type?.value)
    ? weapAttrs.dmg_type?.value
    : Constant.ATK_MODES[atkMode]?.DMG_TYPE || 'blunt';
  let atkForm = Constant.ATK_MODES[atkMode]?.ATK_FORM || 'attack';

  let isPierce = Util.stringMatch(dmgType, 'pierce');
  let isBlunt = Util.stringMatch(dmgType, 'blunt');
  let isSlash = Util.stringMatch(dmgType, 'slash');
  ...
  let attrDmgMod =
    (offhand && attackerRollData[`${dmgAttr}_mod`] > 0
      ? Math.floor(attackerRollData[`${dmgAttr}_mod`] / 2)
      : attackerRollData[`${dmgAttr}_mod`]) || 0;
  ...
  const allowedWeapProfs = Util.stringMatch(attackerRollData.allowed_weap_profs, 'all')
  ...
  if (targetActor) {
    ...
    const aimPenalties = Object.fromEntries(
      validAreas.map((a) => {
        // TODO only show aim penalties for Util.stringMatch(targetActor?.type, 'character') || Util.stringMatch(targetRollData.type, 'humanoid')
        let hitLocTableName =
          Util.stringMatch(atkForm, 'swing') || Util.stringMatch(atkForm, 'attack') ? 'SWING' : 'THRUST';
        if (atkHeight === 'high' || atkHeight === 'low') {
          hitLocTableName += `_${atkHeight.toUpperCase()}`;
        }
  ...
  const weaponRange = +weapAttrs.range?.value; // TODO check if string with ', if so remove, and parse as number in feet, if just a number, multiple by grid size
  ...
  if (options.showModDialog && !options.shownModDialog) {
    const fields = [{ label: 'To-hit modifiers', key: 'dialogAtkMod' }];
    if (!attacker.skipDmgDialog)
      fields.push({ label: 'Damage modifiers', key: 'dialogDmgMod', placeholder: 'e.g. x2, +3d6' });
    return Dialogs.modDialog(options, 'Attack', fields, () => attack(attacker, target, options));
  }
  let dialogAtkMod = {},
    dialogDmgMod = {};
  try {
    dialogAtkMod.formula =
      options.dialogAtkMod && (await new Roll(options.dialogAtkMod).evaluate()) ? options.dialogAtkMod : 0;
    if (/^[\+|\-|\*|\/]/.test(dialogAtkMod.formula)) dialogAtkMod.includesSign = true;
    dialogDmgMod.formula =
      options.dialogDmgMod && (await new Roll(options.dialogDmgMod).evaluate()) ? options.dialogDmgMod : 0;
  ...
  const fluidShield = actorItems.some(
    (i) =>
      i.type === 'item' &&
      i.data.data.worn &&
      !!i.data.data.attributes.shape?.value &&
  ...
  const targetWeapSpeedItem = !targetHeldItems?.length
    ? null
    : targetHeldItems.reduce((a, b) =>
        (+b.data.data.attributes.speed?.value || 0) > (+a.data.data.attributes.speed?.value || 0) ? b : a
      );
  const targetWeapSpeedItemAtkStyle = targetWeapSpeedItem?.data.data.atk_style || 'stable';
  let targetWeapSpeed = !targetWeapSpeedItem
    ? 10 - targetSize
    : +targetWeapSpeedItem.data.data.attributes.speed?.value || 0; // TODO max speed 10? or 8
  targetWeapSpeed += targetWeapSpeedItem
    ? Constant.STANCE_MODS[targetWeapSpeedItemAtkStyle]?.speed_mod(targetWeapSpeedItem) || 0
    : 0;
  const targetReachValues = [
    ...new Set(
      targetHeldItems
        ?.map((i) =>
          i.data.data.attributes.reach?.value
            .split(',')
            .map((n) => Number(n))
            .filter((t) => !isNaN(t))
        )
  ...
  const targetMaxReach = targetReachValues.length
    ? Math.max(...targetReachValues) * Constant.GRID_SIZE
    : targetDefaultReach;
  ...
  const adjTokens = (token, disposition) => {
    return canvas.tokens.objects.children.filter(
      (t) =>
  ...
  let atkText = Util.stringMatch(atkForm, 'attack')
    ? ` ${pluralize(weapName)}`
  ...
  await (async () => {
    if (!isNaN(targetAc)) {
      ...
      if (
        ...
        isCountering &&
        targetWeapSpeedItem &&
        (await Util.rollDice('d6')) + targetWeapSpeed > (await Util.rollDice('d6')) + weapSpeed
      ) {
      ...
      if (Util.stringMatch(targetActor?.type, 'character') || Util.stringMatch(targetRollData.type, 'humanoid')) {
        ...
        // fluid parrying
        isFluidParrying =
          parryItem && parry.fluid_parry_bonus >= targetParryBonus && Util.stringMatch(parryItemHeight, endHeight);
        ...
        sortedWornArmors =
          targetRollData.ac[coverageArea].sorted_armor_ids?.map((id) => targetActor.items.get(id)) || [];
        ...
        if (largeShieldLocs.includes(coverageArea)) {
          ...
          const adjLargeShields = adjFriendlyTokens
            .map((t) =>
        ...
        if (flexWeapon) {
          ...
          sortedWornArmors = sortedWornArmors.filter((i) => !i.data.data.attributes.shape?.value);
      ...
      resultText += `${isBrutalHit ? ' brutally hard' : ''} at ${!aimArea && targetName ? `${targetName}'s` : `the`}${
        hitLoc ? ` ${hitLoc}` : ``
      } (${Util.chatInlineRoll(totalAtk)} vs. AC ${targetAc})`;
      ...
      if (isHit) {
        ...
        if (isBrutalHit && appliedArmors.length) {
          await (async () => {
            ...
            lastLayerDr = Constant.armorVsDmgType[armorMaterial]?.[dmgType].dr;
        ...
        if (doImpale) {
          const maxTargetImpales = coverageArea
            ? appliedArmors.length + Constant.HIT_LOCATIONS[coverageArea].max_impale
            : targetSize + 1;
          ...
          for (let i = 0; i < maxImpales; i++) {
            ...
            if (appliedArmors.length) {
              ...
              if (coverageArea) {
                penMaterial =
                  Util.stringMatch(coverageArea, 'skull') ||
                  (Util.stringMatch(coverageArea, 'chest') && !isShooting && !isPierce)
                    ? 'cuir bouilli'
                    : penMaterial;
              } else {
                const hide = targetActor.data.data.attributes.hide?.value;
                penMaterial = Constant.armorVsDmgType[hide] ? hide : penMaterial;
                console.log('hide', hide);
              }

              const penFlesh = !totalImpaleDmg
                ? await penetrateArmor(penMaterial, dmgType, weapPen, lastLayerDr)
        ...
        // convert slash and pierce damage to blunt if blunting armor remains
        // blunting armor is bulky if blunt, metal if slash, and metal and bulky if pierce
        const bluntingArmor = appliedArmors.find(
          (i) =>
        ...
        if (isKnockdown) {
          ...
          (() => {
            ...
            if (dropKnockdownAreas.includes(coverageArea) && Constant.HIT_LOCATIONS[coverageArea]?.bilateral) {
              ...
              const heldItem = targetActor.items.find(
                (i) => i.data.data[`held_${side}`] && !i.data.data[`held_${otherSide}`]
              );
        ...
        // bleeding
        const attrBleedChance =
          isSlash && isSwing ? weapCutting * 2 : isSpiked && isSwing ? weapCutting : totalImpaleDmg ? weapCutting : 0;
        ...
        if (doBleed) {
          // slash weapons cut through non-blunting armor first
          ...
          // determine severity
          let bleedDesc =
            isBlunt && intBleedAreas.includes(coverageArea) && !totalImpaleDmg
              ? Constant.internalBleedDesc(coverageArea)
              : !isBlunt && totalImpaleDmg > 5 && doubleBleedAreas.includes(coverageArea)
              ? majorBleedDesc
              : minorBleedDesc;
        ...
        const injuryDmgType = !isBlunt && isShooting ? 'pierce' : dmgType;
        ...
        const isShield = !!deflectingArmor?.data.data.attributes.shape?.value;
        if (!isRiposteParrying && !isFluidParrying) {
          // TODO rule for not being able to riposte natural attack from much larger enemy and for not being able to counter such a natural attack
          // use shield if worn in fluid style
          const isFluid = Util.stringMatch(deflectingArmor?.data.data.shield_style, 'fluid');
          const shieldHeight = deflectingArmor?.data.data.held_height || '';
        ...
        const targetDexMod =
          targetDead || targetHelpless || targetIncapacitated || isShooting
            ? 0
        ...
        const parryHeightPenalty = Util.stringMatch(parryItemHeight, endHeight)
          ? 0
          : Util.stringMatch(parryItemHeight, 'mid') || Util.stringMatch(endHeight, 'mid')
          ? 2
          : 4;
        ...
        if (isRiposteParrying && totalAtkResult < unarmoredAc + targetParryBonus - parryHeightPenalty) {
          ...
          if (
            parryItem &&
            parryItem.data.data.attributes.fragile?.value &&
            (await Util.rollDice('d100')) <= Constant.WEAP_BREAK_CHANCE
          ) {
          ...
          totalAtkResult < unarmoredAc &&
          (Util.stringMatch(targetActor?.type, 'character') || Util.stringMatch(targetRollData.type, 'humanoid'))
        ) {
          const dodgeVerb = Constant.HEIGHT_AREAS.high.includes(coverageArea)
            ? 'ducks'
            : atkForm === 'thrust'
            ? 'sidesteps'
            : atkForm === 'swing'
          ...
          if (Constant.armorVsDmgType[hide] && !deflectingArmorName && coverageArea !== 'eye') {
            const hideDesc = Util.stringMatch(hide, 'leather')
              ? `${hide}y`
              : Util.stringMatch(hide, 'scale')
              ? `${hide.replace('e', '')}y`
              : `${hide}`;
          ...
          missDesc = ` ${
            deflectingArmorName
              ? ` but the ${isShooting ? 'missile' : 'blow'}${
                  isShooting && isWood ? ` thunks into ` : isShield ? ` is deflected by` : ` glances off`
                } ${deflectingArmorName}`
              : ' but misses'
          }`;
          // check for weapon item breakage
        ...
        if (!immuneFumbles && (await Util.rollDice('d100')) <= fumbleChance) {
          ...
          const fumbles = [`slips and falls`, `stumbles, leaving them open to attack`];
    ...
    let baseDmgText = `${
      attacker.dmgMulti ? `(${weapDmgResult}${dmgModText})*${attacker.dmgMulti}` : `${weapDmgResult}${dmgModText}`
    }`;
    ...
    if (dialogDmgMod.formula) {
      ...
      totalDmgText = /^[\*|\/]/.test(dialogDmg)
        ? `(${baseDmgText})${dialogDmg}${critDmgModText}`
        : `${totalDmgText}${dialogDmg}${critDmgModText}`;
    }
    ...
    let dmgText = ` for ${Util.chatInlineRoll(totalDmgText)}${dr ? ` (DR ${dr})` : ''}${
      dmgType ? ` ${dmgType}` : ''
    } damage`;

    // injury level
    // consider any shot weapon as pierce for the purpose of inch desc and injury determination
    const injuryDmgType = !isBlunt && isShooting ? 'pierce' : dmgType;
    ...
    // or injuryDmg = Math.min(totalDmgResult - targetHp, totalDmgResult)
    ...
    if (isHit) {
      ...
      if (target.totalDmg > targetHp) {
        ...
        } else if (Util.stringMatch(injuryDmgType, 'slash') || Util.stringMatch(injuryDmgType, 'pierce')) {
  ...
  const flavor =
    attacker.flavor ||
    chatMsgData.flavor ||
    (Util.stringMatch(atkForm, 'attack')
      ? `${weapName}, `
      : `${weapName}${atkStyle && atkStyle !== 'stable' ? ` ${atkStyle}` : ''} ${atkForm.toLowerCase()}${rangeText}${
          atkHeight ? ` ${atkHeight}${endHeight && endHeight !== atkHeight ? `-to-${endHeight}` : ''}` : ''
        }` + `${targetActor ? ` vs. ${targetName}` : ''}`);
  const bubbleString =
    attacker.bubbleString ||
    chatMsgData.bubbleString ||
    (Util.stringMatch(atkForm, 'attack')
      ? `${attackerName} ${pluralize(weapName)}${targetActor ? ` ${targetName}` : ''}`
  ...
  if (attacker.kill)
    Util.playVoiceSound(Constant.VOICE_MOODS.kill, attackingActor, token, { push: true, bubble: true, chance: 0.7 });
...
function formatMods(modsArr) {
  ...
  return modsArr
    .filter((m) => m)
    .reduce((prev, curr) => (curr < 0 ? prev + `-${Math.abs(curr)}` : prev + `+${curr}`), '');
}

function getPenInchesDesc(
  dmgType,
  coverageArea,
  targetSize = 2,
  specDmg,
  ...
  if (injuryDmg) {
    injuryDmg = Util.stringMatch(dmgType, 'pierce')
      ? injuryDmg
      : Util.stringMatch(dmgType, 'slash')
      ? Math.ceil(injuryDmg / 2)
  ...
  if (Util.stringMatch(dmgType, 'slash')) {
    const inchDmg = dmg * 2;
  ...
  if (Util.stringMatch(dmgType, 'pierce')) {
...
async function penetrateArmor(armorMaterial, dmgType, weapPen, lastLayerDr) {
  const armorBaseAc = Constant.armorVsDmgType[armorMaterial]?.base_AC || 0;
  const armorAc = armorBaseAc + (Constant.armorVsDmgType[armorMaterial]?.[dmgType].ac || 0);
...
function armorAbsorption(appliedArmors, armorDented, dmgType, armorUpdates, isSpiked = false) {
  ...
  if (absorbed) {
    ...
    if (!armorDented) {
      ...
      if (baseAc < 2) {
        // TODO use condition instead of base AC
...
function measureRange(token1, token2) {
  const canvasDistance =
    token1 && token2
      ? canvas.grid.grid.constructor.name === 'SquareGrid'
        ? canvas.grid.measureDistanceGrid(token1.position, token2.position)
        : canvas.grid.measureDistance(token1.position, token2.position)
      : undefined;
